...
{{- define "bjw-s.common.lib.pod.field.initContainers" -}}
  ...
  {{- range $key, $containerValues := $componentObject.initContainers -}}
    ...
    {{- if $containerEnabled -}}
      ...
      {{- $containerObject := (include "bjw-s.common.lib.container.valuesToObject" (dict "rootContext" $rootContext "componentObject" $componentObject "containerType" "init" "id" $key "values" $containerValues)) | fromYaml -}}
